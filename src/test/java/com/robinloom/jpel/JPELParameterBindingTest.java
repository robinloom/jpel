...
class JPELParameterBindingTest {
    ...
    void scalarParameterInComparison() {
        ...
        Expression expr = JPEL.compile("persons[age >= :minAge]");
    ...
    void stringParameterComparison() {
        ...
        Expression expr = JPEL.compile("persons[name == :expectedName]");
    ...
    void literalFirstParameterComparison() {
        ...
        Expression expr = JPEL.compile("persons[:minAge <= age]");
        List<Person> adults = expr.setParameter("minAge", 18).getResultList(party, Person.class);
    ...
    void parameterInListMembership() {
        ...
        Expression expr = JPEL.compile("persons[name in :allowedNames]");
    ...
    void parameterInNotInListMembership() {
        ...
        Expression expr = JPEL.compile("persons[name not in :excludedNames]");
    ...
    void multipleParametersInCondition() {
        ...
        Expression expr = JPEL.compile("persons[age >= :minAge && name != :excluded]");
    ...
    void parameterInCollectionOperatorPredicate() {
        ...
        Expression expr = JPEL.compile("orders[items.any(price > :threshold)]");
    ...
    void literalOnlyExpressionWithoutParametersStillWorks() {
        ...
        Expression expr = JPEL.compile("persons[age >= 18]");
    ...
    void parameterWithCombinedLogicalConditions() {
        ...
        Expression expr = JPEL.compile("persons[age > :minAge && (name == :name1 || name == :name2)]");
        List<Person> filtered = expr.setParameter("minAge", 18)
    ...
    void parameterInStringOperation() {
        ...
        Expression expr = JPEL.compile("persons[name startsWith :prefix]");
